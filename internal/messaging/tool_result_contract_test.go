...
func TestRegisteredToolsShareResultContract(t *testing.T) {
	...
	validArgs := map[string]map[string]any{
		...
		"propose_pipeline_run": {"run": map[string]any{
			...
			"orchestrator": map[string]any{"backend": "codex", "model": "gpt"}, "dedicated_assignments": map[string]any{},
		}},
		"get_assigned_task":           {},
		"create_task":                 {"display_name": "sample", "instruction": "do work"},
		"cancel_task":                 {"task_id": "task_1"},
		"list_tasks":                  {},
		"get_task":                    {"task_id": "task_1"},
		"retry_task":                  {"task_id": "task_1"},
		"rearm_task":                  {"task_id": "task_1", "arms": []any{}},
		"wait_for_tasks":              {"execution_handle": "attempt_1", "tasks": []any{map[string]any{"task_id": "task_2", "after_revision": 1}}},
