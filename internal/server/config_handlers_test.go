...
// TestProjectsAutoGeneratedID covers FS-04.A11/R31: POST without a project id
// derives one from the title, an empty title still fails cleanly, and an
// explicit id is honored unchanged.
func TestProjectsAutoGeneratedID(t *testing.T) {
	srv := testServer(t, false)
	h := srv.routes()

	// No id supplied: server derives "<slug>-<timestamp>" from the title.
	rec := doRequest(t, h, http.MethodPost, "/api/projects", map[string]any{
		"title": "AgentDeck Demo",
		"cwd":   "/tmp",
	})
	if rec.Code != http.StatusCreated {
		t.Fatalf("POST without id status = %d body=%s, want 201", rec.Code, rec.Body)
	}
	var created projectResponse
	if err := json.Unmarshal(rec.Body.Bytes(), &created); err != nil {
		t.Fatalf("body: %v", err)
	}
	if !config.ValidSlug(created.ProjectID) {
		t.Fatalf("derived id %q is not a valid slug", created.ProjectID)
	}
	if !strings.HasPrefix(created.ProjectID, "agentdeck-demo-") {
		t.Fatalf("derived id = %q, want agentdeck-demo-<ts> prefix", created.ProjectID)
	}
	if _, err := srv.configStore.ReadProject(created.ProjectID); err != nil {
		t.Fatalf("derived project not persisted under %q: %v", created.ProjectID, err)
	}

	// Empty title with no id: title-required error, not a cryptic slug error.
	rec = doRequest(t, h, http.MethodPost, "/api/projects", map[string]any{"cwd": "/tmp"})
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("empty title status = %d, want 400", rec.Code)
	}
	var verr map[string]any
	json.Unmarshal(rec.Body.Bytes(), &verr)
	errs, _ := verr["errors"].([]any)
	for _, e := range errs {
		if m, ok := e.(map[string]any); ok && m["code"] == "invalid_slug" {
			t.Fatalf("empty-title create surfaced invalid_slug; want only title-required: %s", rec.Body)
		}
	}

	// Explicit id is still honored (API/CLI keep control).
	rec = doRequest(t, h, http.MethodPost, "/api/projects", map[string]any{
		"project": "explicit-id",
		"title":   "Whatever",
		"cwd":     "/tmp",
	...
	if rec.Code != http.StatusCreated {
		t.Fatalf("explicit-id status = %d body=%s, want 201", rec.Code, rec.Body)
	}
	json.Unmarshal(rec.Body.Bytes(), &created)
	if created.ProjectID != "explicit-id" {
		t.Fatalf("explicit id not honored: got %q", created.ProjectID)
	}
}
