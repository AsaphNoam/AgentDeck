...
[`spec overview`](../../../docs/specs/README.md), the FS/TS items named by the handoff or request,
[`INVARIANTS.md`](../../../docs/features/INVARIANTS.md) in full — always, regardless of what the
handoff names — and workflow §§3–7 completely. Check whether code matches the specifications and
whether the specifications cover the code across the handoff's unreviewed range.

Then sweep the diff against every invariant class and tag each finding with its class number. A
class with no applicable surface in the diff is a result to state, not a step to skip.
