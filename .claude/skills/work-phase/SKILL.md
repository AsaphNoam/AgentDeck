...
4. **Commit at every GREEN checkpoint** — code + `HANDOFF.md` together, **directly on `main`**
   ...
   with a `Co-Authored-By:` trailer naming the model that did the work. **Push to origin at session
   exit** — no approval needed; only force-pushes require an explicit human request.
