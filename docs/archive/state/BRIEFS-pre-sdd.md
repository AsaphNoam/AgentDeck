...
### 2026-07-12 — usability review: canonical Phase 0–7 end-to-end

The canonical top-to-bottom review replaced the earlier API-only “10 flows” claim with the repository's full 12-journey matrix. Both shipped Go variants, all 94 UI tests and the production UI build are green; the real browser also passed fresh first paint, fake-agent launch, streaming chat and console-error checks. It found one new release blocker: after a first agent launches, failure of the separate write that marks onboarding complete is deliberately handled as success, dismissing the wizard without an error or retry even though onboarding can return after reload. Live reload also confirmed one-sided chat history and exposed assistant replies splitting into individual stream-delta cards; static sweeps added nine advisories around missing interface styling, silent force-delete/clipboard failures, and CLI readiness/compatibility diagnostics. Full matrix and evidence: [`usability-review-run-2026-07-12-canonical-e2e.md`](usability-review-run-2026-07-12-canonical-e2e.md).

Browser control stalled on the Stop confirmation, then the localhost approval service exhausted its quota. The affected J5–J12 visual/restart variants are recorded as an acceptance gate—not inferred as passing—while focused permission, terminal, archive fallback and UI tests remained green. The report, evidence, handoff and this brief are written and pass `git diff --check`, but the required state commit is still pending because the same quota rejected Git escalation.

**Needs attention:** New/changed — **BLOCKING onboarding completion failure is hidden** and **the review-state commit is pending**. Carried BLOCKING — **crash-restart lifecycle orphan reaping** and **persisted federation watch rehydration**. Carried HUMAN — **Terminal support boundary**, **HTTP-only agent messaging**, **Immediate/prompt-based UI**, **Runtime-switch fallbacks**, **Unbounded transcript indexing**, **Agent env inheritance by design**, **Local API trusts same-machine callers**, **Detached config-source import deferred**, and **API/model compatibility**. Real-backend/federation acceptance and the quota-gated E2E remainder are still open.

**Next:** Agent — first commit the existing state-only changes as `usability review: canonical Phase 0-7 E2E — state recorded`; then run `/fix-review` for all three BLOCKING findings before 7.9 and replay the gated J5–J12 checkpoints when browser/localhost access is available.
