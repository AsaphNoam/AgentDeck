...
- **ADVISORY — recipient unread badge does not update live on message arrival.** `internal/server/server.go` message-inserted sink calls only `eventBus.SetSnapshot(update)` for the recipient (`toAgentID`) while it `PublishStateUpdate`s for the sender. No SSE `state_update` is pushed for the recipient, so its card's `unread_messages` badge doesn't appear until the agent's next *unrelated* published state change (e.g. the nudger waking an idle recipient, or the agent finishing). A busy recipient shows no "mail pending" indicator in the meantime — weakening the §7.1 / acceptance "recipient cards show a message indicator when mail is pending." **Fix:** `PublishStateUpdate` the recipient too (`Touch` already recomputes `UnreadMessages`; no state transition → no spurious notification). **Test:** insert a message for an idle/busy recipient, assert a `state_update` with `unread_messages>0` is published for it.
- **ADVISORY — `budget_exceeded` notification has no agent name/address.** The `SetBudgetExceededSink` payload in `internal/server/server.go` is built inline with a generic `"Agent hit its message budget"` title and omits `agent_name`/`address`, unlike `bus.notificationPayload` which already has a proper `budget_exceeded` case. The user sees a nameless toast. **Fix:** route the budget breach through `notificationPayload` using the agent's snapshot so the toast names the agent.
