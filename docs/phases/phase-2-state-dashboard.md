...
Turn the single-agent loop into a supervisable, multi-agent dashboard. Add the file-watcher **state manager** and the multiplexed **SSE event bus**, then build the React card-grid home view that renders every agent live. After this phase you can run several agents at once and watch their status change in real time, reorder cards, and open a chat panel.
...
### In scope
- State manager: fsnotify watcher over `running/` and `status/`; recompute affected agent state and emit SSE on change.
...
- Subscribe to `running/` and `status/` with fsnotify.
- On write/create/delete, recompute the agent's effective state (merge identity + running + status) and emit a `state_update` event.
- Debounce rapid successive writes to the same file.
- On startup, do a full scan so the dashboard reflects already-running agents.
...
- Resilient reconnect on the client (last-event tracking; full state resync on reconnect since the file store is the source of truth).
...
- Cards update live from `state_update` within ~1s of the underlying file change.
...
- [ ] Closing and reopening the dashboard resyncs current state from disk (no stale cards).
