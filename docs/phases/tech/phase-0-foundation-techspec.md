...
Stand up the skeleton every later phase hangs off of: the typed Go **file-store** over `~/.agentdeck/`, a Go HTTP **server** bound strictly to `127.0.0.1:{port}`, the `agentdeck` **CLI** (`dashboard start|stop|open`, `--version`), seed-data bootstrap, and an `install.sh` that builds the binary + UI. No agents run, no runtimes, no SSE, no file watching, no config UI — those are Phases 1–3.

**In scope:** repo/package layout; lazy `~/.agentdeck/` creation; typed file store (Read/Write/List/Delete, atomic writes, `AGENTDECK_HOME` + `~` expansion, `agent_id` generation, corrupt-file survival); GET-only `/api/*` endpoints sourced from the store; CLI skeleton with pidfile/detach; seed data (4 roles, 1 example project, `backends.json` v2, `config.json` v1) that never clobbers existing data; `install.sh`.

**Out of scope:** ACP/runtimes/launch (P1), state manager + SSE (P2), config CRUD/onboarding (P3), all `POST`/`PUT` mutation endpoints.
...
| Logging | **stdlib `log/slog`** with a `slog.NewJSONHandler` to stderr | "Structured logging" (req 4.2) without a dependency. JSON lines are greppable and machine-readable; level via `AGENTDECK_LOG_LEVEL` (default `info`). |
...
| UI build | **Vite 5 + React 18 + TypeScript 5** (scaffold only) | Master PRD §7. Phase 0 only scaffolds (`npm create vite@latest ui -- --template react-ts`) and wires the build so `install.sh` produces `ui/dist`. No app logic. |
...
| Atomic writes | stdlib only: `os.CreateTemp` + `f.Sync()` + `os.Rename` | No dependency needed; rename is atomic on the same filesystem on macOS/Linux. |
...
Dependency budget for Phase 0: **cobra**, **pkg/browser**. Everything else is stdlib.
...
AgentDeck/
  ...
  README.md                       prereqs + quickstart (Go 1.22+, Node 18+, npm, python3)
  ...
  internal/
    ...
    store/
      store.go                    Store struct, New(home), Home resolution
      paths.go                    AGENTDECK_HOME + ~ expansion, per-object path builders
      layout_dirs.go              EnsureLayout(): create all dirs lazily, never clobber
      types.go                    all data structs (§4) with json tags
      ...
      config.go                   ReadConfig/WriteConfig (single file)
    ...
    store/
      testdata/                   corrupt JSON fixtures (note: lives at internal/store/testdata/)
        corrupt_role.json
  ...
  mcp/                            Node MCP messaging server — SCAFFOLD ONLY (Phase 5 implements)
    package.json
    README.md                     "Placeholder; implemented in Phase 5"
...
- Test fixtures live in `internal/store/testdata/` (Go's `testdata` convention — ignored by the build, accessible from tests in the package).

---

## 4. Data structures (Go)

All structs live in `internal/store/types.go`. JSON tags match master PRD §3 exactly. Use pointers for nullable/override fields so `null` vs absent vs value is distinguishable.
...
`internal/store` exposes a single `*Store` value holding the resolved home directory. Per-object methods follow one shape. Generic helpers in `atomic.go` back them.
...
// layout_dirs.go
func (s *Store) EnsureLayout() error   // mkdir -p every dir in §3; idempotent; never deletes/overwrites
...
Per-type methods (pattern shown for Agent; Role/Project/Status/Running identical, keyed by their id; Backends/Config/Layout are single-file Read/Write):

```go
// agents.go
func (s *Store) ReadAgent(id string) (Agent, error)        // ErrNotFound if absent; ErrCorrupt if unparseable
func (s *Store) WriteAgent(a Agent) error                  // atomic
func (s *Store) ListAgents() ([]Agent, error)              // skip corrupt entries (log+continue), never error whole list
func (s *Store) DeleteAgent(id string) error               // os.Remove; ErrNotFound tolerated as nil

// NewAgentID generates "a_" + 6 lowercase hex chars from crypto/rand,
// retrying on collision against existing agents/{id}.json (max 10 tries).
...
// MustLocalAddr builds the listen address and refuses anything but loopback.
...
- `New(st *store.Store, cfg store.Config, log *slog.Logger) *Server`.
- `Start(ctx context.Context) error`:
  ...
  4. Block on `ctx.Done()` (SIGINT/SIGTERM via `signal.NotifyContext`), then `srv.Shutdown(timeoutCtx)` (5s), remove pidfile if owned.
...
| `GET /api/sessions` | `handleSessions` | `store.ListRunning()` joined w/ `ListAgents()` → `[]` when empty |
| `GET /api/roles` | `handleRoles` | `store.ListRoles()` |
| `GET /api/projects` | `handleProjects` | `store.ListProjects()` |
| `GET /api/backends` | `handleBackends` | `store.ReadBackends()` |
| `GET /api/layout` | `handleLayout` | `store.ReadLayout()` (default if absent) |
| `GET /` and `GET /{path...}` | `handleStatic` | embedded UI FS; SPA fallback to `index.html` |

All `/api/*` handlers wrapped by `middleware(cors, requestLog)`. Unmatched `/api/*` → 404 JSON. Non-GET to a GET route → mux returns 405 automatically.
...
- `agentdeck dashboard start [--port N] [--detach]`:
  - Resolve store home, `EnsureLayout()`, `SeedIfAbsent()`, `ReadConfig()` (port flag overrides config.Port for this run only; not persisted).
  - **Foreground (default):** install `signal.NotifyContext`, write pidfile (`{home}/dashboard.pid` with current PID + chosen port), call `server.Start(ctx)`, remove pidfile on exit.
...
`internal/store/seed.go` → `SeedIfAbsent()`. Called after `EnsureLayout()`. Each item written **only if the target path does not already exist** (`os.Stat` → `IsNotExist`). Never overwrites.
...
- `List*` skip corrupt files (log + continue) — one bad file never breaks a listing.
- Single-file getters (`ReadConfig`/`ReadBackends`/`ReadLayout`): on `ErrCorrupt` the **handler** falls back to in-memory defaults and logs; the server never crashes. `dashboard start` on corrupt `config.json` logs WARN and uses the default config (port 4317).
- A corrupt agent/role/project file is treated as absent for that id.
...
Active agents = join of `running/*.json` with `agents/*.json`. Empty store → `[]` (never `null`).
```json
[
  {
    "agent_id": "a_8f3c12",
...
### `GET /api/roles` → 200
Map of role id → Role. Seeded store returns the 4 seed roles.
```json
{
  "implementer": { "title": "Implementer", "system_prompt": "Implement the requested changes carefully; write tests; keep diffs focused.", "skip_permissions": null },
  "reviewer":    { "title": "Reviewer",    "system_prompt": "Review changes for correctness, edge cases, and consistency.", "skip_permissions": null },
...
Map of project id → Project. Seeded store returns `my-app`.
```json
{
...
The full `backends.json` (version 2). Corrupt/missing → in-memory default (the seed above) + WARN log; still 200.
```json
{ "version": 2, "backends": { "claude": { "...": "..." }, "codex": { "...": "..." } } }
```

### `GET /api/layout` → 200
The `layout.json`; missing/corrupt → default.
```json
...
- **Corrupt `config.json`:** log WARN, use default config (port 4317), continue. Do not rewrite the corrupt file (avoid clobbering user data they may want to fix).
- **Corrupt single role/project/agent file:** treated as absent; excluded from `List*`; logged once.
- **Empty store (fresh `~/.agentdeck`):** `/api/sessions` → `[]`; `/api/roles` & `/api/projects` → seeded (because `start` seeds before serving); `/api/layout` & `/api/backends` → seeded.
...
- **Atomic write interrupted:** temp file remains; never visible under the real name (rename is the commit point); a leftover temp is overwritten/cleaned next write. Temp files use a `.tmp-*` prefix in the same directory so rename stays on one filesystem.
- **`agent_id` collision:** `NewAgentID` retries up to 10×; after that returns an error (astronomically unlikely with 24 bits of hex).
- **Disk full / permission denied on write:** `writeJSONAtomic` returns the error; the caller surfaces it (CLI exits non-zero; handler → 500). No partial file is left under the real name.
...
1. **Module bootstrap.** `go mod init github.com/agentdeck/agentdeck`; set `go 1.22`; add cobra + pkg/browser; commit `Makefile` with `build/test/ui/run`.
2. **`internal/version`.** `Version/Commit/Date` vars; ldflags wiring in Makefile/install.sh.
3. **`internal/store/types.go`.** All structs from §4. *Verify:* `go vet` clean.
4. **`internal/store/paths.go` + `store.go`.** Home resolution (`AGENTDECK_HOME` → `~/.agentdeck`), `ExpandTilde`, `New()`. *Verify:* unit test resolution + tilde.
5. **`internal/store/atomic.go` + `errors.go`.** `writeJSONAtomic`, `readJSON`, sentinels. *Verify:* write→read round-trip test; corrupt → `ErrCorrupt`.
6. **`internal/store/layout_dirs.go`.** `EnsureLayout()` idempotent mkdir of all dirs. *Verify:* run twice, no error, no clobber.
7. **Per-object methods.** `agents.go` (+`NewAgentID`), `running.go`, `status.go`, `roles.go`, `projects.go`, `backends.go`, `config.go`, `layout.go`. *Verify:* round-trip test per object; `List*` skips corrupt.
8. **`internal/store/seed.go`.** `SeedIfAbsent()` writing the §5.4 set only when absent. *Verify:* seed into temp home, assert files + contents; re-seed after mutating a file → unchanged (no clobber).
9. **`internal/server`**: `bind.go` (+ loopback assertion), `json.go`, `middleware.go`, `handlers.go`, `routes.go`, `server.go`, `static.go`. *Verify:* `httptest` against each route; bind-address test.
10. **`internal/cli`**: `root.go` (+ `--version`), `dashboard.go` (start/stop/open), `pidfile.go`, `launch_stub.go`. *Verify:* `agentdeck --version`; start in foreground binds + `/api/health` 200; stop kills via pidfile.
11. **`cmd/agentdeck/main.go`.** Wire cobra root, execute. *Verify:* `go build ./cmd/agentdeck`.
12. **UI scaffold.** `npm create vite ui --template react-ts`; `App.tsx` pings `/api/health`. *Verify:* `cd ui && npm run build` → `ui/dist`.
13. **Embed UI.** `static.go` `//go:embed ui/dist`; SPA fallback; `dev` build tag → disk. *Verify:* `go build` after `ui/dist` exists serves `/`.
14. **`mcp/` scaffold.** `package.json` + README placeholder. *Verify:* present, no build wiring required yet.
15. **`install.sh`.** Build UI (`npm ci && npm run build`), build Go binary with ldflags, install to `~/.local/bin` or `/usr/local/bin`, run `agentdeck dashboard start`-free seed by invoking a hidden `agentdeck init` (or rely on first `start`). *Verify:* clean clone → `./install.sh` → `agentdeck --version`.
16. **README prereqs.** Document Go 1.22+, Node 18+, npm, python3, quickstart.
17. **Full acceptance pass.** Walk the phase-0 §5 checklist end to end.
...
**Store unit tests (`internal/store`):**
- **Round-trip every object:** for `Agent, RunningEntry, Status, Role, Project, BackendsConfig, Layout, Config` — construct → `Write*` → `Read*` → `reflect.DeepEqual`. Table-driven.
- **Corrupt-file survival:** write garbage bytes to a role file → `ReadRole` returns `ErrCorrupt` (not panic); `ListRoles` skips it and returns the rest. Use `internal/store/testdata/corrupt_*.json`.
...
- **`NewAgentID`:** matches `^a_[0-9a-f]{6}$`; collision retry covered by pre-creating a file with the first deterministic id (seed `crypto/rand` via interface or just assert format + uniqueness across 1000 calls).
...
Target: store + server packages at meaningful coverage; the three named test classes (round-trip, corrupt survival, AGENTDECK_HOME isolation) and the bind test are mandatory per acceptance criteria.

---

## 10. Interfaces produced for later phases

**File store (`internal/store`)** — the typed substrate every phase reads/writes:
- `*Store` with `New()`, `Home()`, `EnsureLayout()`, `SeedIfAbsent()`.
- Typed CRUD: `Read/Write/List/Delete` for `Agent, RunningEntry, Status, Role, Project`; `Read/Write` for `BackendsConfig, Config, Layout`.
...
- Atomic-write guarantee: Phase 2's file watcher can assume readers never see partial files (write-temp-then-rename).

**Server skeleton (`internal/server`)**:
- `New(store, config, logger)` + `Start(ctx)` lifecycle that Phase 1 extends with `POST` routes and Phase 2 extends with `/api/events` (SSE).
...
**Conventions locked:** `~/.agentdeck/` layout, `AGENTDECK_HOME` override, RFC3339 timestamps, filename-as-id (`roles/{id}.json`), version fields (`config` v1, `backends` v2).

---

## 11. Resolved decisions

**Q: Default port?** → **`4317`.** Confirmed as the project default (MAP.md placeholder + phase-0 §3 seed). `config.json.port` is the source of truth; `--port` flag overrides per-run without persisting. Reasoning: 4317 is unassigned by IANA for common dev servers, avoids clashes with Vite (5173), and is already baked into the seed.

**Q: Embed UI assets in the binary vs. serve from disk?** → **Embed** (`//go:embed ui/dist`). Locked recommendation per phase-0 §6. Rationale: the master PRD's single-binary distribution promise (§7) is only honest if the UI ships inside the binary — `install.sh` produces one artifact, `dashboard start` serves the UI with no separate static-file path or web server. A `dev` build tag falls back to serving `ui/dist` from disk so Vite HMR works during development (run `vite dev` on :5173 and hit the Go API via CORS; or serve disk assets for an integrated test). SPA routes fall back to `index.html`.

**Q (implied): roles/projects shape over the wire — array or map?** → **Map keyed by id**, matching on-disk filename-as-id and PRD JSON. `sessions` stays an array (ordered running list).

**Q (implied): nullable role override.** → `skip_permissions` is `*bool` so `null` (inherit), `true`, and `false` are all distinguishable on disk and over the API.
