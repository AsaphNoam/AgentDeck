...
- **Previous state:** All open lifecycle-claim findings are fixed. `LaunchStage` now takes the exclusive
  ...
  reserves every member claim before stopping any: a busy member returns one retryable `409` and no
  member stops, while a successful release owns every stop-and-cleanup operation (FS-02.R20/A8,
  TS-03.R27, **INV §4/§5**). Deterministic slow-ACP coverage proves pipeline launch retains its
  registration under a concurrent Stop; group coverage proves a conflict stops neither the claimed
...
- **Previous state:** The projects-home background menu now covers the full available canvas, so a
  ...
  for (`ClaimPendingWakeMail` → `wake_attempted`) before spawning anything and records its outcome on
  exactly those ids (`SetDeliveredVia`), so a handshake-successful adapter that dies before its first
  nudge is not respawned every sweep, and mail arriving mid-wake stays `pending` and re-arms
  (FS-06.R23/A12, TS-04.R26). `MarkUnreadDeliveredVia`/`MarkRead` accept `wake_attempted` so the
  delivering nudge or a direct read restamps it honestly. **Wake gates:** `wakeCandidate` returns a
  typed `*runtime.APIError` and distinguishes "excluded" from "could not be evaluated", so a corrupt
  project definition surfaces `500` instead of the no-handle `404` and fails the directory read
  rather than silently omitting the agent (FS-01.R33, TS-03.R25). **Addressable set:**
  `state.AddressableAgents` is one query over one snapshot with per-row availability, and both it and
  `StoppedWakeCandidates` embed one shared `stoppedWakeGates` predicate, so a Stop mid-read can no
  longer list an agent twice or produce a false `role@project` ambiguity (TS-04.R26).
  **Proposals:** `ProposeRun` canonicalizes the request id to the derived proposal id before digest
  and persistence; a proposal is consumed only after its approved Save/Start commits (migration 15
  adds `consumed_at`), re-proposing the same content re-arms the one record, retention keeps the
  newest 100, and one unreadable record no longer aborts the approval list (FS-14.R33/A13,
  TS-09.R26, TS-02.R22). **Context meter:** a mid-turn ACP `usage_update` now republishes through the
  existing status write+touch seam, so a long turn no longer shows a stale percentage.
  **Smaller:** file search reports storage failures instead of calling them `agent_not_running`
  (TS-03.R24), and the `list_agents`/`send_message` MCP descriptions now say stopped wakeable agents
  are addressable. New regressions: six in `internal/server`
  (`TestStopDuringWakeConflictsAndKeepsRegistration`,
  `TestSuccessfulWakeConsumesMailEvenIfAdapterDiesBeforeNudge`,
  `TestFailedWakeLeavesNewerMailPending`, `TestWakeGateFailureSurfacesTypedError`,
  `TestAddressableAgentsNeverDuplicatesAcrossStop`, `TestFileSearchStorageErrorIsNotAStoppedAgent`),
  the new `internal/pipeline/proposals_test.go` and `internal/state/pipeline_proposals_test.go`,
  `TestUsageUpdateRepublishesContextPctMidTurn`, and one `AgentDeckerBuilder` case. Each was
  confirmed to fail against the pre-fix code. A `slowACP` test helper (a shell launcher that stalls
...
The 2026-08-23 re-review found twelve **Must fix** and two **Worth fixing** findings after the first
nine were closed. The review covered every cited planned requirement, surrounding shipped
lifecycle/activation/pipeline/context requirements, all invariants, and the concrete registry,
launch/resume/stop, activation, pipeline-report, and turn-end seams. The independent audit
corroborated active deletion, borrowed-runtime recovery, dependency activation, dashboard attention,
and re-arm eligibility. The change remains Waiting to start; no product code, specification, or
ready-change file changed.
...
- 2026-08-23 — Re-reviewed dependency-aware work after the first nine findings were resolved and
  ...
  ownership on delete/cancel/person-result paths; recovery reaping a borrowed personal runtime;
  contradictory assignment timing and retry identity; an undefined start-attempt policy and
  dependency activation instruction; false pipeline/task result convergence; missing pipeline-run
  deletion fan-out; and an impossible existing-agent target contract for agent-created tasks. Two
  stale lines still contradict the intended recovery and dashboard-attention behavior. The shipped
  code checks confirmed the registry non-adoption premise, the mail-only hard-coded activation
  instruction, the existing pipeline-specific report transaction, and the single hard-coded turn-end
  consumer. No product code, specification, or ready-change file changed.
...
- 2026-08-22 — review: design-reviewed the waiting `pull-based-context-links.md` change against
  ...
  `index.searchableText` and the UI `foldTranscript` reducer with no seam named (**INV §2**), and
  `current_pipeline_report` being reachable only inside the reporting turn given
  `CurrentPipelineAttemptForAgent` plus TS-09.R10/R11. Six **Worth fixing**: write-only seen/hidden
  projection, R14 identity-deletion machinery for an operation with no route, durable invisible
  grants over credential-bearing transcript data with no owner surface or retention, the
  "never silently omitted" promise against `transcript` dropping >8 MiB records, a second undefined
  `source_unavailable` outcome, and neither transcript selector covering the turn the caller is
  completing. The design's committed factual claims that hold — `ResolveRecipient` reuse,
  `PRAGMA foreign_keys=ON`, immutable attempt reports, budget isolation — are recorded as not
  promoted. `make check-specs` and `git diff --check` pass; no product code or specification changed.
...
- 2026-08-22 — review: verified the fix for the six re-review findings and found one regression it
  ...
  attempted boundary now commits immediately before `composeResumeSpec` (a stopped agent pointed at a
  removed backend keeps its pending opportunity), `ReleaseMailActivation` is a delete-or-update pair
  in one transaction (the earlier `UNIQUE constraint failed: activations.agent_id` probe now returns
  nil and strands nothing), durably ineligible recipients are discarded instead of re-claimed every
  two seconds, the `nudgeState`/`nudgeOnce` shim and `NudgeInterval` are gone
  (`ActivationSweepInterval`), the FS-06.A11 assertion is now `len(waiting) == 0`, and the dead
  wake-claim state API and its `nudge`/`wake_attempted`/`wake_failed` constants are removed with the
  read-marking `CASE` simplified. Specs moved with the code (FS-06.R10/R27/A15, FS-07 §6, TS-01.R21,
  TS-02.R23). One new **Must fix**: the new eligibility gates apply the stopped-only
  pipeline-association exclusion to running recipients too, so a running pipeline-associated agent
  accepts mail and is never activated for it — confirmed by running the same scenario in a worktree
  at `9862682` (1 provider prompt) and at `1fd26ed` (0). Invariant sweep over the fix diff —
  applicable: §1, §2, §4, §5, §7, §9, §10, §15; §2 produced the finding (a second Go spelling of the
  wake-gate predicate drifted from `stoppedWakeGates`), and §1/§4/§5/§7/§9/§10/§15 were sound.
  Sections §3, §6, §8, §11, §12, §13, §14 have no applicable surface in this diff. `make check-specs`,
  `make test`, `make build`, `git diff --check`, and focused `-race` on the activation, wake, and
  state paths all pass. No product code or specification was changed by this review.

- 2026-08-22 — fix: closed every remaining mail-activation finding. **INV §5** — the stopped
  activation attempt now occurs immediately before `composeResumeSpec`, after every fallible,
  side-effect-free resume gate; a missing backend keeps its mail opportunity pending. **INV §1/§6**
  — the executor discards a claimed opportunity when its recipient is terminal, archived,
  project-archived, or pipeline-associated, while keeping mail unread; a gate-evaluation failure
  still releases for retry. **INV §5/§7** — transactional release deletes an old claim when newer
  mail already owns the pending row, otherwise releases it, and every caller reports release errors.
  **INV §10** — removed the obsolete wake-claim APIs, test shim, and old activation vocabulary;
  strengthened A11 to prove a failed attempt re-arms nothing. FS-06, FS-07, TS-01, and TS-02 match
  these boundaries. Focused state/server regressions, `make check-specs`, `make test`, `make build`,
...
- 2026-08-18 — Fixed both open Must-fix findings (**INV §4/§5**). `LaunchStage` now holds
  ...
  failed initial send; `TestPipelineLaunchStageRespectsLifecycleClaim` uses slow ACP to prove a
  concurrent ordinary Stop receives `409` and leaves the hook token, MCP registration, and hook
  settings intact. Group release now reserves every member lifecycle claim before it stops any, so
  contention returns one `409 conflict` and stops nobody instead of returning a partial `200`;
  FS-02.R20 and TS-03.R27 define the retry path, and
  `TestReleaseGroupRespectsLifecycleClaim` confirms both members remain running. `make check-specs`,
  both Go test modes, focused `-race`, `make build`, and `make dist` pass. No UI code changed.
...
- 2026-08-17 — Fixed all twelve findings from the review of stopped-agent wake, pipeline proposals,
  ...
  exactly their pending rows before spawning; wake-gate failures remain typed errors; the complete
  addressable directory comes from one snapshot; run proposal request ids are canonical; approved
  proposals are consumed and bounded; malformed proposals no longer hide valid ones; mid-turn ACP
  usage updates republish immediately; file-search storage errors stay storage errors; and messaging
  descriptions include stopped wakeable agents. The owning FS/TS requirements and regression tests
  cover each correction. `make check-specs`, both Go test modes, focused lifecycle/wake race tests,
  `make build`, all 227 UI tests, presentation/style checks, and `make dist` pass. No live-browser
  pass was run.
