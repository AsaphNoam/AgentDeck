...
| J3 | **First agent launch + chat round-trip** | `seeded/` + fakeacp | Create agent, send a message, streamed response renders, card status transitions (idle → busy → idle) match reality. Once diagram rendering ships (FS-03.R37): have the agent reply with a fenced `mermaid` block, confirm it stays a code block while streaming and becomes a themed diagram once the fence closes, toggle it back to source, reload and reopen the archived transcript to confirm the same result, and check it under Core and Sky & Grove. Send a diagram carrying a script element and an HTML label and confirm neither reaches the page. |
