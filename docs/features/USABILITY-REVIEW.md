...
| J5 | **Grid & layout** | `seeded/` | Verify each card shows the configured project title (not its durable id) and every context meter, including zero, visibly identifies its percentage. Reorder, density, groups, collapse; persists across page reload **and** server restart. Delete an agent that is in the saved order — grid stays sane. Empty grid renders a real empty state. |
