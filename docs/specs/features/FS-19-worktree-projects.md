...
**Status:** Current
**Code:** `internal/worktree/`, `internal/server/worktree.go`, `internal/config/worktrees.go`, `internal/state/worktrees.go`, `ui/src/features/dashboard/` · **Journeys:** J16
**Absorbed:** —
...
- **R1.** Any active project whose expanded `cwd` resolves inside a Git working tree
  ...
  **Branch** pre-filled as `agentdeck/<slug(title)>`, and an editable **Base** pre-filled from the
  ...
  not launch an agent.
- **R2.** Every project has an optional `base_branch` (FS-04.R45). When it is empty, the
...
- **R5.** Agents, tasks, and pipeline stages change nothing: they launch into the
  ...
  **is** two projects.
- **R6.** A worktree project is visibly a worktree project: its dashboard card and its
...
- **R8.** Archiving a worktree project with an owned checkout offers — never forces —
...
- **R9.** The **New worktree project** action is absent when the source project's `cwd`
  ...
  anything is created.
- **R10.** Creation is all-or-nothing ahead of setup: if the branch already exists, the
...
- **R11.** Forking a worktree project is allowed and behaves identically: the new branch
  still comes off the effective base branch by default, not off the source project's branch.

## 5. Acceptance criteria

- **A1.** From a repo-backed project, the fork action creates branch + owned worktree +
...
- **A2.** A failing `setup_command` still yields a launchable project and a visible
  warning containing the captured output. Verified by test.
- **A3.** With the owned checkout deleted out-of-band, the next launch recreates it on
...
- **A4.** The archive dialog on a dirty owned checkout shows the uncommitted-changes
...
- Fork, checkout resolution, setup, status, and consented deletion: `internal/server/worktree.go`;
  routes in `internal/server/worktree_handlers.go`. Architecture in TS-12.
- Start-path wiring (R5/R7): `composeLaunchWithOptions`, `composeResumeSpecWithGeneration`, and
  `composeSwitchSpec`; the read-only pipeline pre-flight in `ValidateStage`.
- Creation form (R1): `ui/src/features/dashboard/WorktreeForkDialog.tsx`.
- Entry points, branch display, and the archive consent (R6/R8): `ProjectDashboard`,
  `ScopedProjectView`, and `ProjectArchiveDialog` in
  `ui/src/features/dashboard/ProjectDashboard.tsx`; `.project-card-branch` and
  `.scoped-project-header` in `ui/src/styles/features/dashboard.css`.
- Project fields (R2/R3): FS-04.R45.
- Regression anchors: `internal/server/worktree_projects_test.go`
  (`TestWorktreeForkCreatesBranchCheckoutAndProject`, `TestWorktreeForkOfAForkUsesTheEffectiveBase`,
  `TestWorktreeForkFailingSetupStillCreatesTheProject`, `TestWorktreeForkRejectionsLeaveNothingBehind`,
  `TestWorktreeCheckoutRecreationAndMissingBranch`, `TestConcurrentCheckoutRecreationClaimsOnce`,
  `TestArchiveDeletesCheckoutOnlyWithConsent`, `TestExternalCheckoutIsNeverDeleted`,
  `TestWorktreeStatusAndListEnrichment`), `internal/worktree/git_test.go`, and the worktree cases in
  `ui/src/features/dashboard/ProjectDashboard.test.tsx` and
  `ui/src/features/dashboard/ScopedProjectDashboard.test.tsx`.
