...
- **R16** `(planned)` — **An agent that goes away without reporting leaves the task interrupted.**
...
- **R17** `(planned)` — **A restart ends every task runtime rather than adopting it.** AgentDeck
...
- **A9** `(planned)` (R7, R21) — Fifteen tasks becoming ready with the budget set to four produce
...
- **A12** `(planned)` (R18) — Deleting a prerequisite parks a dependent whose arm was still waiting,
