...
**Status:** Partial
**Code:** planned · **Journeys:** planned pipeline journey
**Absorbed:** —
...
Every requirement in this specification is unshipped and reflects the confirmed product decisions
in §6.

### 2.1 Templates and starting a run

- **R1 `(planned)`.** A user can create, read, edit, and delete a reusable **pipeline template** with
...
- **R2 `(planned)`.** Starting a run assigns a configured backend and model to every stage, so the
...
- **R3 `(planned)`.** Before starting a template, the user may set a run display name and must set a
...
- **R4 `(planned)`.** A run can be started from the dashboard and through a local CLI/API surface.
...
- **R5 `(planned)`.** AgentDeck launches only the current stage's agent. Its initial assignment names
  ...
  launched early.
- **R6 `(planned)`.** A running stage agent can report exactly one authoritative result for its
...
- **R7 `(planned)`.** Runtime status alone never completes a stage: `idle`, `done`, or process exit
...
- **R8 `(planned)`.** An accepted result is durable and visible before the next stage starts. The
...
- **R9 `(planned)`.** A template maps each `success` or `failure` outcome to another stage or a final
...
- **R10 `(planned)`.** A typical quality template can express **Work → Review → Validate → Complete**,
...
- **R11 `(planned)`.** The Pipelines surface shows each run's goal, project, current stage and agent,
...
- **R12 `(planned)`.** A launch failure, current-agent crash before a result, or `blocked` result
  ...
  earlier attempt.
- **R13 `(planned)`.** Stopping a run cancels/stops its current agent through the ordinary lifecycle
...
- **R14 `(planned)`.** Dashboard restart restores every non-finished run to its last durable stage,
...
- **R15 `(planned)`.** A pipeline stage uses the selected role's ordinary permission policy and the
...
- **R16 `(planned)`.** Agents from a run remain normal AgentDeck agents: they have stable agent ids,
...
- **R17 `(planned)`.** Template validation rejects missing/duplicate stage ids, unknown roles,
...
- **R18 `(planned)`.** Starting a run revalidates its effective stage selections. A template whose
...
- **R19 `(planned)`.** A stage report with an unknown/stopped caller, caller not assigned to the
...
- **R20 `(planned)`.** Continue never converts a missing result into success. It resumes only a
  ...
  appropriate.
- **R21 `(planned)`.** Concurrent runs retain AgentDeck's existing shared-project-directory
...
- **R22 `(planned)`.** A template declares named required or optional run inputs, and each stage
...
- **R23 `(planned)`.** The Start Run form and API collect all required run inputs before launch.
...
- **R24 `(planned)`.** Before accepting a stage result whose selected transition starts another
...
- **R25 `(planned)`.** A dedicated **Pipelines** page lists reusable templates and active/completed/
...
- **R26 `(planned)`.** Create with AgentDecker first lets the user choose one configured, chat-capable
...
- **R27 `(planned)`.** The AgentDecker builder accepts a natural-language pipeline description, asks
...
- **R28 `(planned)`.** Runs can be started and controlled from the Pipelines page and local CLI/API.
...
- **R29 `(planned)`.** Pipeline-level notifications use the existing toast/desktop/mute pipeline for
  ...
  **completed** (terminal success or terminal failure). Ordinary successful stage transitions do not
  notify.
- **R30 `(planned)`.** The AgentDecker Save/Start confirmation is an interaction guard, not a new
...
- **A1 `(planned)`** — A person creates and edits one model-neutral four-stage template, starts it
...
- **A2 `(planned)`** — A fake four-stage run launches exactly one chat agent at a time with the
  ...
  with fake runtimes.
- **A3 `(planned)`** — Idle/done without a result never advances; an authenticated current-stage
...
- **A4 `(planned)`** — A stage-reported failure follows its configured transition; launch failure,
...
- **A5 `(planned)`** — Restart at each transition boundary restores one current attempt and never
  loses a result or launches a duplicate agent. *Verify:* persistence/restart integration matrix.
- **A6 `(planned)`** — Pipeline launch uses normal permission and backend validation, and a missing
...
- **A7 `(planned)`** — In a browser journey, a person starts Codex Work with Claude Review/
  Validation, follows live stage status, resolves a pause, observes a repair loop, and opens every
  stage transcript from the completed run. *Verify:* a new pipeline usability journey.
- **A8 `(planned)`** — Concurrent same-project runs are allowed only after the start surface makes
  ...
  tests and the pipeline usability journey.
- **A9 `(planned)`** — Run-start text inputs and one stage's named outputs resolve into a later
...
- **A10 `(planned)`** — Create with AgentDecker offers configured backend/model choices for the
  ...
  fake-runtime server test and Pipelines UI journey.

## 6. Deviations & open decisions

No pipeline behavior is shipped. The human has confirmed these product boundaries:
...
a separate cross-backend launch capability. The human confirmed the complete feature behavior on
2026-07-25; every item remains planned until implementation ships.
