...
- **R6.** `GET/POST /api/projects`, `PUT /api/projects/{project}`, `DELETE /api/projects/{project}`
  ...
  slug rule and immutability as role ids (R3).
...
- **R18.** The Project step creates the user's first real project (preferred over the seeded
  `my-app`, whose `cwd` may not exist), and its slug is carried into the Launch step's default
  selection.
...
- **R22.** If `onboarding_complete` is already `true`, `satisfied` is forced true regardless of the
  ...
  credential failure.
- **R23.** The gate is satisfied for rendering purposes when any of: the server reports
  `satisfied`, `onboarding_complete` is true, or the wizard set a session-local "dismissed" flag on
  completion. Once satisfied, the dashboard renders and the wizard does not reappear.
...
- **A10.** The onboarding wizard walks a fresh install from backend credentials → project → first
  running agent, gating the dashboard throughout. *Verified:* journey J2.
...
- **Validation:** `internal/config/validate.go` (`ValidSlug`, `ValidateRole`, `ValidateProject`).
