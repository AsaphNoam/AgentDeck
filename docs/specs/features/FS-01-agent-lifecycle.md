...
- **R34** — **Stop and resume are one exclusive transition per agent.** Stop (R6), Release group
  ...
  settles (the client is not obliged to retry automatically; TS-03.R25); a Release group member in
  that position is reported as not stopped in that action's per-agent result. Either way the
  in-flight resume completes normally with its registration intact. Without this, a stop could not
...
- **A18** (R33–R34) — A Stop issued while a wake is inside its resume returns `409`, the wake
  ...
  torn down. A Release group covering that agent behaves the same way: the member is reported as not
  stopped, the wake keeps its registration, and the retried release stops it. A stopped agent whose
