...
export function ToolResult({ event }: { event: TranscriptEvent }) {
  ...
  const state = isError ? "error" : empty ? "empty" : expanded ? "expanded" : "collapsed";
  const status = isError ? "Failed" : "Completed";
  return (
    <article className={`tool-block tool-result ${isError ? "tool-result-error" : ""}`} data-ui="tool-result" data-state={state}>
      {empty ? <span data-slot="content" className="tool-result-status">{status}</span> : <pre data-slot="content">{shown}</pre>}
      {!empty && long && (
