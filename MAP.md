...
| [docs/phases/BRIEFS.md](docs/phases/BRIEFS.md) | **Human-facing digests.** The only file the human reads; every session ends by prepending a ≤250-word plain-language brief (workflow §10). |
...
| [docs/phases/USABILITY-REVIEW.md](docs/phases/USABILITY-REVIEW.md) | **Behavior-driven usability review protocol.** Top-to-bottom journey-based review of the *running* app (fresh/seeded/lived-in fixtures, real binary, real browser) with token-efficient subagent orchestration. Complements the AGENT-WORKFLOW §8 static diff review; findings feed the same `/fix-review` loop. |
